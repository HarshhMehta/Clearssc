...
const Banner = () => {
  ...
  const { token } = useContext(AppContext); // ✅ Get login token from context

  if (token) return null; // 🔒 Hide banner if user is logged in

  return (
    <div
      style={{ backgroundColor: "#141B31" }}
      className="flex h-[250px] md:h-[450px] rounded-lg mx-12 sm:mx-24 md:mx-28 lg:mx-32 px-6 sm:px-10 md:px-12 lg:px-14 my-20"
    >
      {/* left side */}
      <div className="flex flex-col items-center justify-center md:flex-1 md:py-16 lg:py-24 lg:pl-5 ml-5">
        <div className="text-center text-xl sm:text-2xl md:text-3xl lg:text-5xl text-white font-semibold">
          ...
          <p className="mt-6 md:mt-10">
            With 100% <span className="text-green-500">Trusted </span>Plans
          </p>
        </div>
        <button
          onClick={() => {
